...
The gauge identity gives more than an upper bound on the number of strict regions. Their mean can be computed from a triangular matrix whose size depends only on the width. The same matrix treats independent nondegenerate Gaussian biases by restoring one extra index.
...
With nondegenerate independent biases, each new preactivation is either a proper affine hyperplane on an old open cell or a nonzero constant there, almost surely. Thus the strict partitions refine: their counts are nondecreasing, and any change of their geometric open cells increases the count. Monotone convergence and the finite mean limit imply a finite integer limiting count almost surely, so only finitely many refinements occur and its expectation is $\rho_n$.
\end{proof}
