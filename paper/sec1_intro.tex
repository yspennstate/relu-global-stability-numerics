...
The weighted identity also gives an exact depth rate at fixed width: both global survival and the mean strict-region count have logarithmic rate $\log(1-2^{-n})$ (\cref{cor:fixed-width-depth}). At width two the mean strict count has the closed form in \cref{prop:width-two-mean}. These concern strict preactivations and a different order of limits from the threshold theorem.
