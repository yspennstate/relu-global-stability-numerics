...
\end{enumerate}
Every estimate is finite in $n$ and $L$. The only place a limit enters is the definition of $\alpha_L$ itself, and the asymptotic form (iv), where an expansion in $\theta$ is convenient. \Cref{sec:numerics} checks the identity of step 3 and the bounds of steps 4--6 against brute-force computation at small width, and \cref{sec:sharp} shows that the finite bound $h_\alpha$ has the exact large-$n$ exponent of the scalar moment, so the derivative $\log(\alpha/2)$ and the coefficient $\tfrac52$ are not artifacts of a loose estimate.
