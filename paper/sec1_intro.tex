...
At fixed width, the weighted identity gives a stronger depth asymptotic: both global survival probability and the mean strict-region count, divided by $(1-2^{-n})^L$, increase to finite positive limits (\cref{cor:fixed-width-depth}). At width two the mean strict count has the closed form in \cref{prop:width-two-mean}. These concern strict preactivations and a different order of limits from the threshold theorem.
...
Region counts and collapse at initialization are also established topics. Hanin and Rolnick~\cite{hanin2019} study mean region counts along one-dimensional subspaces and the volume of region boundaries. Wang~\cite{wang2022}, Theorem~2.1, bounds expected linear-region counts for several architectures, including width two, with independent continuously distributed symmetric weights and biases and scalar output. Lu, Shin, Su and Karniadakis~\cite{lu2020} analyze the probability that a randomly initialized ReLU network is constant, including its depth and width dependence under symmetric initialization. Our width-two formula in \cref{prop:width-two-mean} concerns strict activation regions of a bias-free vector-output network; ordinary constant cells and affine-bias region counts are different quantities. Rister and Rubin~\cite{rister2021} give geometric bounds on whole-network survival and a relative-convergence argument for finite data in a growing-width regime. Lu et al., Theorem~3.5 and Appendix~C, give one-dimensional-input bounds and a one-ray state analysis closely related to the fixed-width argument here. \Cref{cor:fixed-width-depth} controls the normalized strict-region mean and survival probability on the full square input space; its proof compares these earlier settings explicitly.
...
\end{enumerate}
Every estimate is finite in $n$ and $L$. The only place a limit enters is the definition of $\alpha_L$ itself, and the asymptotic form (iv), where an expansion in $\theta$ is convenient. \Cref{sec:numerics} checks the identity of step 3 and the bounds of steps 4--6 against brute-force computation at small width. The scalar function $g_\alpha$ does two jobs: it bounds the moment at every finite width, and \cref{sec:sharp} shows that it is also its exact limiting exponent. Thus no asymptotic approximation is needed in the finite theorem.
