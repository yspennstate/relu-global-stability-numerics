...
At fixed width, the same identity gives an exact finite matrix formula for the mean strict-region count, with separate formulas for zero biases and independent nondegenerate Gaussian biases (\cref{thm:region-transfer}). In the zero-bias case, both global survival probability and the mean strict count, divided by $(1-2^{-n})^L$, increase to finite positive limits; the mean constant is an explicit rational number (\cref{cor:fixed-width-depth}). With nondegenerate biases, the expected count instead increases to a finite rational limit (\cref{cor:affine-region-limit}). These are statements about strict activation regions and a different order of limits from the threshold theorem.
