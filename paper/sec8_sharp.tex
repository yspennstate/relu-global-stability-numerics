...
\subsection{The depth rate at fixed width}
\label{sec:fixed-width-depth}

The same identity also controls the opposite order of limits: keep the width fixed and let the depth grow. Write $R_{n,L}=\#\{D:R_D\ne\varnothing\}$ for the strict-region count and $S_{n,L}=\{F^{(\alpha)}_{n,L}\not\equiv0\}$ for global survival. These are different objects: a strictly negative final preactivation can leave a nonempty strict region whose output is zero.

\begin{corollary}[fixed-width depth rate]\label{cor:fixed-width-depth}
Fix $n\ge1$ and $\alpha>0$, with independent Gaussian layers, no biases, input domain $\R^n$ and ReLU at the output as in \cref{eq:forward}. Put $\beta_n=1-2^{-n}$. Then
\begin{align}
...
\end{align}
At depth one, $R_{n,1}=2^n$ almost surely. In particular,
...
\begin{proof}
Sum \cref{eq:orbit} with functional $1$ over all masks. If $D_j=0$ at an intermediate layer $j<L$, then $H^D_{j+1}=0$ and $r(\im A_D)=0$. There remain $2^n(2^n-1)^{L-1}$ masks, since the final mask is arbitrary. Bounding each orthant count by $C_{n,L}$ gives the upper bound in \cref{eq:depth-strict}, also at $L=1$.

For the lower bound restrict to $\rank D_1=1$, all subsequent intermediate masks nonzero, and arbitrary $D_L$. The invertible first matrix gives $n$ independent row hyperplanes and hence $2^n$ regions. After $D_1$ selects one row, every later formal preactivation row is a nonzero multiple of that row almost surely. Indeed, conditional on the earlier layers, a fresh Gaussian row acts on a nonzero retained vector and has probability zero of giving zero; a nonzero mask retains a nonzero vector for the next step. A finite union over these rows and masks preserves the almost-sure statement. The later rows create no new hyperplane, so $r(\im A_D)=2^n$ for this family. Its $n(2^n-1)^{L-2}2^n$ masks contribute exactly
\[
2^{-nL}\,n(2^n-1)^{L-2}\,2^n\,2^n=n\beta_n^{L-2}.
\]

For survival, positive homogeneity gives $F(0)=0$, so $S_{n,L}=\{K_{n,L}(\alpha)>0\}$. By \cref{lem:reduction}, this event requires a realized strict mask with $J_D\ne0$. Apply \cref{prop:orbit} with the invariant functional $\one_{\{J_D\ne0\}}$ and take a union bound. A zero mask at any layer, including the last, kills $J_D$. At most $(2^n-1)^L$ masks contribute, giving $\PP(S_{n,L})\le C_{n,L}\beta_n^L$.

For the lower bound, $W_1$ is invertible almost surely. Choose the input $s=W_1^{-1}e_1$, which depends only on $W_1$ and gives $x_1(s)=e_1$. Conditional on any nonzero current state, the next independent layer has $n$ independent centered nondegenerate Gaussian preactivations, so its ReLU output survives with probability $\beta_n$. The matrices $W_2,\ldots,W_L$ are independent of the first-layer witness; the tower property therefore gives survival probability $\beta_n^{L-1}$ along this input. Its survival implies global survival. For comparison, a deterministic nonzero input survives all $L$ layers with probability exactly $\beta_n^L$.

Finally, at fixed $n$, $C_{n,L}$ is a positive polynomial in $L$ of degree $n-1$ (the constant $2$ if $n=1$). Thus $L^{-1}\log C_{n,L}\to0$, and the positive lower and upper bounds give \cref{eq:depth-rate}.
\end{proof}

At width one the two bounds coincide: $\E R_{1,L}=2^{2-L}$ and $\PP(S_{1,L})=2^{1-L}$. At $L=2$ there is one strict region for every nonzero scalar pair, but global survival has probability $1/2$. This small case explains why the final mask must be treated differently in the two upper bounds. The rate statement fixes $n$ and supplies neither a ratio asymptotic nor uniformity in a growing width.

Earlier work already establishes death at large depth. In the zero-bias square setting, Rister and Rubin~\cite{rister2021}, Sections~3--4, give a global survival upper bound with base $1-2^{-n^2}$ and a fixed-input lower bound with base $\beta_n$. Lu et al.~\cite{lu2020}, Theorem~3.5, already obtain bounds implying the rate $\log\beta_n$ for one-dimensional inputs, with a separate final affine readout. \Cref{cor:fixed-width-depth} controls the whole square input space $\R^n$ and also the mean strict-region count. The earlier geometric bounds already imply almost-sure eventual death and finite expected death depth; those qualitative consequences are not claimed as new here.
...
\begin{proof}
Sum \cref{eq:orbit} with functional $1$ over all formal masks: $\E R_{2,L}=4^{-L}\sum_D\E r(\im A_D)$. If an intermediate mask $D_j$ is zero, $j<L$, the next preactivation matrix has a zero row and $r(\im A_D)=0$.

Otherwise let $k$ be the first index $j<L$ for which $D_j$ has rank one, or put $k=L$ if all intermediate masks are $I_2$. Through layer $k$, the preactivation matrices are $Q_1,\dots,Q_k$, since all earlier masks are the identity. Their $2k$ row directions are distinct almost surely, by the joint-density argument used for \cref{eq:all-active}. If $k<L$, multiplication by the rank-one mask retains one row of $Q_k$; every later nonzero row is proportional to that row and creates no new direction. These later rows are nonzero almost surely: at each step a fresh Gaussian row acts on a nonzero retained vector, and its zero set has conditional probability zero. Thus in either case the arrangement in the input plane has $2k$ distinct lines, giving $r(\im A_D)=4k$ almost surely.

For $k<L$ there are two choices for the first rank-one mask, three nonzero choices at each later intermediate layer, and four choices for the final mask. This gives $8\cdot3^{L-k-1}$ masks of type $k$. There are four masks of type $k=L$. Consequently
\[
\E R_{2,L}=4^{-L}\left[32\sum_{k=1}^{L-1}k3^{L-k-1}+16L\right]
=8\frac{3^L-1}{4^L},
\]
where the last finite-sum identity follows by differentiating a geometric sum (and also holds for $L=1$, when the sum is empty).
\end{proof}
