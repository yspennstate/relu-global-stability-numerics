...
The finite theorem rests on the bound $h_\alpha$ of \cref{lem:h}, obtained from one deterministic inequality. This section computes the exact exponential rate of the scalar moment and shows that $h_\alpha$ has the same derivative at zero and the same quadratic coefficient. It then records a few facts about the structure of the problem.
...
For every $\alpha>0$ and $\theta>0$ the limit
\[
g_\alpha(\theta)=\lim_{n\to\infty}\frac1n\log\E[(X_n^{(\alpha)})^{\theta n}]
\]
exists and equals $\max_{0\le u\le1}\Phi_{\alpha,\theta}(u)$. It extends continuously to $\theta=0$ by $g_\alpha(0)=0$, and $g_\alpha'(0+)=\log(\alpha/2)$. Near the critical variance, writing $\alpha=2e^{-\delta}$,
\begin{equation}\label{eq:g-expansion}
g_{2e^{-\delta}}(\theta)=-\delta\theta+\frac52\theta^2+O(\theta^3),
\end{equation}
the same expansion as $h_{2e^{-\delta}}$ in \cref{eq:h-delta}. Taking the limit in the finite-width bound of \cref{lem:h} gives $g_\alpha(\theta)\le h_\alpha(\theta)$ for $0<\theta<1/4$, their common domain, and the two agree to second order at zero.
\end{proposition}

\begin{proof}
The limit and its formula follow from the discussion above. At $\theta=0$ the last three terms of \cref{eq:Phi} cancel, leaving $H(u)-\log2$, whose unique maximizer is $u=\tfrac12$ with value $0$, since $H'(u)=\log\frac{1-u}u$ vanishes only there and $H''<0$; so $g_\alpha(0)=0$, and joint continuity of $(u,\theta)\mapsto\Phi_{\alpha,\theta}(u)$ makes $g_\alpha$ continuous at $0$.

For the right derivative, let $u_\theta$ maximize $\Phi_{\alpha,\theta}$. Outside any neighbourhood of $\tfrac12$ the function $H(u)-\log2$ is bounded above by a negative constant, so for small $\theta$ a maximizer cannot stay outside it: $u_\theta\to\tfrac12$. Evaluating at the old maximizer, $g_\alpha(\theta)\ge\Phi_{\alpha,\theta}(\tfrac12)$, so $\liminf_{\theta\downarrow0}(g_\alpha(\theta)-g_\alpha(0))/\theta\ge\partial_\theta\Phi_{\alpha,0}(\tfrac12)$. Using maximality of $u_\theta$ at $\theta$ and of $\tfrac12$ at $0$, $g_\alpha(\theta)-g_\alpha(0)=\Phi_{\alpha,\theta}(u_\theta)-\Phi_{\alpha,0}(\tfrac12)\le\Phi_{\alpha,\theta}(u_\theta)-\Phi_{\alpha,0}(u_\theta)$; dividing by $\theta$ and using the mean value theorem in $\theta$ together with $u_\theta\to\tfrac12$ and the continuity of $\partial_\theta\Phi$ gives the matching $\limsup$. Now $\partial_\theta\Phi_{\alpha,\theta}(u)=\log(2\alpha)+\log(\tfrac u2+\theta)$, and at $(\tfrac12,0)$ this is $\log(2\alpha)+\log\tfrac14=\log(\alpha/2)$.

For the expansion, $\alpha=2e^{-\delta}$ shifts $\Phi$ by an exact linear term, $\Phi_{2e^{-\delta},\theta}=\Phi_{2,\theta}-\delta\theta$, so $g_{2e^{-\delta}}(\theta)=g_2(\theta)-\delta\theta$. At $(u,\theta)=(\tfrac12,0)$ the second derivatives of $\Phi_{2,\theta}(u)$ are $\Phi_{uu}=-4$, $\Phi_{u\theta}=2$, $\Phi_{\theta\theta}=4$, so with $u=\tfrac12+v$,
\[
\Phi_{2,\theta}(\tfrac12+v)=-2v^2+2v\theta+2\theta^2+O((\abs v+\theta)^3).
\]
The first-order condition $-4v+2\theta+O(v^2+\theta^2)=0$ gives $v=\theta/2+O(\theta^2)$, and substituting, $-2(\theta/2)^2+2(\theta/2)\theta+2\theta^2=\tfrac52\theta^2$. Hence $g_2(\theta)=\tfrac52\theta^2+O(\theta^3)$ and \cref{eq:g-expansion} follows.
\end{proof}

The quadratic coefficient has a probabilistic meaning. For $Y=BZ^2$, $\E Y=\tfrac12$ and $\Var Y=\tfrac54$, and $\Var(Y)/(2(\E Y)^2)=\tfrac52$: the curvature of the moment exponent at the critical point is the normalized variance of one masked Gaussian square. The rate constant $\sqrt{10}$ of \cref{thm:main}(iv) is $\sqrt{2\cdot2\cdot\tfrac52}$, one factor $2$ from $\alpha=2e^{-\delta}$, one from the optimization $\theta=\delta/5$ and value $-\delta^2/10$.
...
At $L=1$ the all-active mask is realized with probability one, $J_D=W_1$ for it, and \cref{eq:reduction} gives $K_{n,1}(\alpha)=\op{W_1}$: every mask $D_1$ gives $\op{D_1W_1}\le\op{W_1}$. By the Bai--Yin theorem \cite{baiyin}, for a square matrix with independent entries of variance $\alpha/n$ and finite fourth moment, $\op{W_1}\to2\sqrt\alpha$ almost surely. Hence $\PP(K_{n,1}(\alpha)\le1)\to1$ for $\alpha<\tfrac14$ and $\to0$ for $\alpha>\tfrac14$, that is,
\[
\alpha_1=\tfrac14 .
\]
...
\subsection{Why the proof never conditions on an input}
