...
\]
and for the bounded functional $F=\min(\op{J_D},1)$. The first version of the test was wrong twice, and both failures are instructive. Sweeping eight thousand directions round the circle instead of enumerating the arcs missed thin arcs and pulled both sides down by about six percent, consistently; sampling a set of arcs is not enumerating it, and the error appeared as a bias rather than noise. And testing with $F=\op{J_D}^2$ produced a paired difference two and a half standard errors from zero at depth three: not a violation, but the signature of a heavy-tailed functional whose average is dominated by rare large draws, so that the usual standard error understates the uncertainty. The identity holds for every nonnegative gauge-invariant $F$, so a bounded one may be chosen for the test, and with bounded $F$ every ratio is within two percent of one and every paired $z$-score within $1.3$ of zero.
