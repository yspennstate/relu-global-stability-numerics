...
An exact identity can be tested, and \cref{eq:orbit} was, at width two. In the plane the sign pattern of $A_Ds$ is constant on each arc between consecutive boundary directions. The arcs are enumerated exactly from the row dependencies of the fixed masks; \Cref{sec:numerics} describes the certified represented-weight geometry and the clipped-norm error bound. \Cref{tab:orbit} reports the paired Monte Carlo test over the weights for $F\equiv 1$, which turns \cref{eq:orbit} into
\[
\PP(R_D\ne\varnothing)=\E\,r(\im A_D)/2^{nL},
\]
and for the bounded functional $F=\min(\op{J_D},1)$. Two points of numerical design. Sampling directions round the circle instead of enumerating the arcs misses thin arcs and biases both sides downward, so the arcs are enumerated. An unbounded functional such as $F=\op{J_D}^2$ is sensitive to rare large draws, so the test uses bounded functionals. The identity holds for every nonnegative gauge-invariant $F$; with the bounded choices used here, every ratio is within two percent of one and every paired $z$-score within $1.3$ of zero.
