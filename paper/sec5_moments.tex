...
\begin{lemma}[optimized finite-width scalar moment bound]\label{lem:scalar}
For every $n\ge1$, $\alpha>0$ and $\theta>0$, the function in \cref{eq:scalar-def} satisfies
\begin{equation}\label{eq:scalar-bound}
\E\bigl[(X_n^{(\alpha)})^{\theta n}\bigr]\le e^{n g_\alpha(\theta)}.
\end{equation}
It extends continuously to $g_\alpha(0)=0$, and
\begin{align}
g'_\alpha(\theta)&=\log\frac{\alpha}{z_\theta^2(1+z_\theta)},\label{eq:scalar-derivative}\\
g_\alpha(\theta)&=\theta\log\frac\alpha2+\frac52\theta^2+O(\theta^3).
\label{eq:scalar-expansion}
\end{align}
as $\theta\downarrow0$, with a remainder independent of $\alpha$.
\end{lemma}

\begin{proof}
Write $\lambda(t)=\log[(1+(1-2t)^{-1/2})/2]$ for $0<t<1/2$.
Set $p=\theta n$ in \cref{lem:xp} and apply it to $S_n$. Since $X_n^{(\alpha)}=\alpha S_n/n$, \cref{eq:mgfS} gives, for every such $t$,
\[
\frac1n\log\E(X_n^{(\alpha)})^{\theta n}
\le \theta\log\frac{\alpha\theta}{et}+\lambda(t).
\]
...
\[
\lambda'(t)=\frac1{z^2(1+z)},\qquad
t\lambda'(t)=\frac{1-z}{2z^2}.
\]
The stationary equation $t\lambda'(t)=\theta$ therefore has solution $z=z_\theta$ from \cref{eq:scalar-def}. At this point $\theta/t=1/[z_\theta^2(1+z_\theta)]$, and substitution gives \cref{eq:scalar-bound}.

Differentiate the minimized expression in $\theta$. Its derivative in $t$ vanishes at the minimizing $t$, so the chain rule leaves $g'_\alpha(\theta)=\log(\alpha\theta/t)$, the derivative in \cref{eq:scalar-derivative}. Since $z_\theta=1-2\theta+O(\theta^2)$, this derivative is $\log(\alpha/2)+5\theta+O(\theta^2)$. The defining formula tends to zero at $\theta=0$; integration gives the expansion. All dependence on $\alpha$ sits in the exact linear term $\theta\log\alpha$.
\end{proof}

The derivative at zero decides the qualitative threshold: if $\alpha<2$ then $g'_\alpha(0+)<0$ and $g_\alpha$ is negative for all small positive $\theta$. At $\alpha=2$ the linear term vanishes. The coefficient $\tfrac52$ is the normalized variance $\Var(Y)/(2(\E Y)^2)=\tfrac{5/4}{2\cdot1/4}$ of one masked Gaussian square; it is where the constant $\sqrt{10}$ of the rate comes from. \Cref{sec:sharp} shows that the entire function $g_\alpha$, not only its expansion, is the exact limiting moment exponent.
